 ...
 */
contract StaticMarketCollection is StaticMarketBase {
    ...
    function ERC20ForAnyERC721WithOneFee(bytes memory extra,
        ...
        returns (uint)
    {
        require(uints[0] == 0, "ERC20ForERC721WithOneFeeForCollection: Zero value required");
        require(howToCalls[0] == AuthenticatedProxy.HowToCall.DelegateCall, "ERC20ForERC721WithOneFeeForCollection: call must be a delegate call");

        (address[3] memory tokenGiveGetAndFeeRecipient, uint256[2] memory priceAndFee) = abi.decode(extra, (address[3], uint256[2]));

        require(priceAndFee[0] > 0,"ERC20ForERC721WithOneFeeForCollection: ERC721 price must be larger than zero");
        // require(addresses[2] == tokenGiveGetAndFeeRecipient[0], "ERC20ForERC721WithOneFeeForCollection: call target must equal address of token to give");
        require(addresses[5] == tokenGiveGetAndFeeRecipient[1], "ERC20ForERC721WithOneFeeForCollection: countercall target must equal address of token to get");
        ...
        returns (uint)
    {
        ...
        // require(addresses[2] == tokenGiveGetAndFeeRecipient[0], "ERC20ForAnyERC721WithTwoFees: call target must equal address of token to give");
