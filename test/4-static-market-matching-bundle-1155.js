...
const { buildParamsForBundle, buildParamsForPlatform, buildBundleData, relayerFeeAddress, royaltyFeeAddress} = require('./test-utils')
...
// describe("WyvernExchange", function() {
    ...
    let deploy_contracts = async () => {
        let exchange, statici, registry, atomicizer, erc1155, transferPlatformToken
        if (TEST_NETWORK == "development") {
            ...
            [exchange, statici] = await Promise.all([WyvernExchange.new(CHAIN_ID, [registry.address], '0x'), StaticMarketBundleForERC1155.new()]);
            [erc20, erc1155, erc721] = await deploy([TestERC20, TestERC1155, TestERC721])
        } else {
            [exchange, statici, registry, atomicizer, erc1155, transferPlatformToken] = [
        ...
        return { registry, exchange: wrap(exchange), atomicizer, statici, erc1155, transferPlatformToken }
    }
