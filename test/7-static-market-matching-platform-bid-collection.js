...
contract('WyvernExchange', (accounts) => {
    ...
    const erc721_for_platform_token_test = async (options) => {
        ...
        let { exchange, registry, atomicizer, statici, erc1155, erc721, wyvernStatic, transferPlatformToken, staticiCollection } = await deploy_contracts()
        ...
        if (hasFee && hasRoyaltyFee) {
            ...
            selectorTwo = web3.eth.abi.encodeFunctionSignature('ETHForAnyERC721WithTwoFees(bytes,address[7],uint8[2],uint256[6],bytes,bytes)')
            ...
            selectorTwo = web3.eth.abi.encodeFunctionSignature('ETHForAnyERC721WithOneFee(bytes,address[7],uint8[2],uint256[6],bytes,bytes)')
        } else {
            selectorOne = web3.eth.abi.encodeFunctionSignature('ERC721ForETH(bytes,address[7],uint8[2],uint256[6],bytes,bytes)')
            selectorTwo = web3.eth.abi.encodeFunctionSignature('ETHForAnyERC721(bytes,address[7],uint8[2],uint256[6],bytes,bytes)')
        }
        ...
        let paramsTwo = buildParamsWithFixedSize(addressesTwo, amountTwo, relayerFee, royaltyFee, hasFee, hasRoyaltyFee)
